    ...
    public class UsersController : BaseController
    {
        ...
        public ActionResult Index()
        {
            ViewBag.countTrash =
                ...
                db.Accounts.Count(a => a.status == "0" && a.Role == "1"); // đếm user status 0 và role 1
            var user = from a in db.Accounts
                where a.status == "1" && a.Role == "1"
        ...
        public ActionResult ChangeRole(int? id) //Phân quyền thành Admin
        {
            var user = db.Accounts.SingleOrDefault(a => a.account_id == id && a.Role == "1");
            if (user == null || id == null)
            {
            ...
            db.Entry(user).State = EntityState.Modified;
            db.SaveChanges();
            Notification.set_flash("Đã phân quyền thành Admin! (ID = " + id + ")", "success");
            return RedirectToAction("Index");
        }
